...
> 과제 요구사항(7. 협업 및 형상관리) 대응 문서 · 최종 확인: 2026-09-09 (룰셋·저장소 설정 API 재조회)
...
| 팀원별 유의미한 커밋 10회+ | 주간 감사 → `docs/commit-audit.md` | ✅ (2026-09-09 전원 달성) |
| 문서와 Git 이력 불일치 없음 | 개인별 작업 요약을 커밋 이력 기준으로 작성 | ✅ (감사 수치·이력 대조 완료) |
...
| `pull_request` → `require_extra_approval_for_unattributed_changes` | `true` | 작성자 불명 변경의 무단 병합 |
...
### 이중 안전장치 — 완료 (2026-09-09)

저장소 설정에서도 Squash를 차단했다. 룰셋(`allowed_merge_methods: ["merge"]`)에 더해
PR 화면의 Squash 버튼 자체가 없어 다른 브랜치 대상 머지나 향후 룰셋 변경 시 실수 여지도 제거된다.
...
Rebase 머지는 역할 카드 #2 결정("Merge/Rebase만")에 따라 허용을 유지한다.
...
- `.env.example`은 **키 이름·설명·안전한 기본값·자리표시자만** 포함, 실제 비밀값 없음 (PDF 제약사항 대응)
