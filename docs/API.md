...
| POST | `/api/auth/logout` | ✕ | 로그아웃 (세션이 없어도 200; 있으면 비움) | 200 |
...
처리 흐름: **질문 수신 → 직전 N개 Q/A 컨텍스트 구성 → AI API 호출 및 응답 수신 → DB 저장 시도 → 사용자에게 응답 반환**

**요청**
```json
{ "question": "내가 방금 뭘 물어봤지?" }
```
- `question`: 필수, 공백만 있으면 불가. 서버 상한은 `MAX_QUESTION_LENGTH`(기본 1000자); 현재 프론트 제한은 별도 1000자
...
| 422 | 빈 질문/설정된 길이 상한 초과(기본 1000자) | Pydantic 검증 오류 |
...
> 타임아웃/실패 시에도 서버는 종료되지 않으며, 실패 이력은 `status=ai_error`로 DB 저장을 시도합니다. DB 자체 오류로 저장에 실패할 수 있습니다.
...
> 채팅 응답의 `status=success`는 AI 응답 성공을 뜻하며 DB 저장 성공을 보장하지 않습니다. `chat_id=-1`은 DB 저장 ID를 얻지 못한 경우입니다.
