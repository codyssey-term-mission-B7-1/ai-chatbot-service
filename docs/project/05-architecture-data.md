...
- **표준 이벤트 38종**, stderr 구조화 출력, `event=` 키: `request_received`, `ai_call_start/success/fail`, `db_save_start/success/fail`, `auth_*`, `login_fail`, `login_rate_limited`, `chat_rate_limited`, `auth_password_reset_*`, `admin_user_deleted`, `thread_created/deleted`, `readyz_db/schema_failure` 등 (docs/LOGGING.md 집계표).
...
| 선택지 | 장점 | 단점 |
|---|---|---|
| **(1) 구조화 표준 이벤트 → stderr (12-factor)** | 컨테이너 로그 수집과 자연 결합(Railway가 수집), 파일 관리 불필요, 이벤트 사전(38종)으로 집계 가능 | 검색·보존 기간이 플랫폼에 의존 |
