...
## 설정 경계 — .env에 넣을 것 / 코드 상수로 남을 것 (#152)

| 구분 | 담는 것 | 예시 | 파일 |
|---|---|---|---|
| **환경설정 (.env)** | 배포 환경마다 달라지는 것 — 운영자가 바꿀 수 있어야 하는 값 | `SESSION_SECRET`, `PASSWORD_PEPPER`, `AI_API_KEY`, `DATABASE_URL`, 한도·스위치(`CHAT_RATE_PER_MIN` 등) | `.env` / `.env.example` |
| **코드 상수 (policies)** | 환경과 무관한 불변 정책 — 운영자가 함부로 완화하지 못해야 하는 값 | 비밀번호 8~64자·72바이트, 보안 헤더·CSP, 로그 마스킹 필드, 문서 경로, 컨텍스트 상한 200 | `app/policies.py` |

- 판단 기준: **"환경을 옮기면 바뀌는가?"** 예 → `.env`에 필드 추가(`app/config.py` Settings) / 아니오 → `policies.py` 상수.
- `.env.example`의 표기: **[필수]** 미설정 시 운영에서 시작 거부 또는 안전하지 않음 / **[운영]** 운영 배포에서만 의미 / **[선택]** 미설정 시 `app/config.py`의 코드 기본값으로 동작.
- 운영 필수값 3종(AI 키·메일·관리자 계정)의 실제 발급·등록 절차는 아래 P0 절차서를 따른다.
