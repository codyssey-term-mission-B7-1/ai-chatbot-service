...
def database_path(explicit: str | None) -> Path:
    if explicit:
        ...
        url = make_url(
            os.environ.get("DATABASE_URL") or values.get("DATABASE_URL") or "sqlite:///./app.db"
        )
...
def backup(
    database: Path, destination: Path | None = None, keep: int = 7, timeout: float = 30.0
) -> Path:
    ...
    try:
        ...
        raise
    generations = sorted(
        (x for x in directory.glob(prefix + "*.db") if x.is_file() and not x.is_symlink()),
