    ...
    async with async_playwright() as automation:
        ...
        page.on(
            ...
            lambda request: (
                ...
                if request.method == "POST" and request.url.endswith("/api/chats")
                else None
            ),
        )
        ...
        await capture(
            "09-password-byte-error.png", "한글 25자 비밀번호의 클라이언트 바이트 제한 안내"
        )
        ...
        assert await page.locator("#question").get_attribute("required") is not None
        ...
        assert len(requests) == before
        await page.locator("#question").dispatch_event(
        ...
        assert (
            await other.post("/api/chats", data={"question": "두 번째 로컬 계정의 질문"})
        ).status == 201
        await other.dispose()
        ...
        await capture(
            "07-admin-logs.png", "로컬 운영자 함수로 명시 부여한 관리자 계정의 실제 조회 화면"
        )
...
def main(argv=None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="chatbot-local-evidence-") as temp:
        os.environ.update(
            {
                "DATABASE_URL": "sqlite:///" + str(Path(temp) / "evidence.db"),
                "DEBUG": "true",
        ...
        result.update(
            {
                ...
                "scope": "LOCAL_SYNTHETIC_NOT_PRODUCTION",
