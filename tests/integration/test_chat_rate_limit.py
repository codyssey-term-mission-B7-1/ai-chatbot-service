...
def test_exceeding_per_minute_cap_returns_429(client, monkeypatch, caplog):
    ...
    assert client.post("/api/chats", json={"question": "첫째"}).status_code == 201
    assert client.post("/api/chats", json={"question": "둘째"}).status_code == 201

    caplog.clear()
    with caplog.at_level(logging.WARNING):
        blocked = client.post("/api/chats", json={"question": "셋째"})
    ...
    assert len(client.get("/api/users/me/chats").json()) == 2
...
def test_zero_disables_chat_rate_limit(client, monkeypatch):
    ...
    for i in range(12):
        response = client.post("/api/chats", json={"question": f"질문 {i}"})
...
def test_rate_limit_applies_per_user(client, monkeypatch):
    ...
    with TestClient(app) as other:
        ...
        assert other.post("/api/chats", json={"question": "하나"}).status_code == 201
        assert other.post("/api/chats", json={"question": "둘"}).status_code == 429
    # 다른 사용자의 한도는 독립적이다.
    assert client.post("/api/chats", json={"question": "별개"}).status_code == 201
...
def test_context_restoration_still_matches_ai_after_commit_release(client, fake_ai):
    ...
    assert client.post("/api/chats", json={"question": "배포 방법"}).status_code == 201
    assert client.post("/api/chats", json={"question": "방금 뭐라고 했지?"}).status_code == 201
    ...
    logs = client.get("/api/users/me/chats?status=success").json()
    assert len(logs) == 2
