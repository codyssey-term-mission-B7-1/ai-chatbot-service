...
def test_chat_with_foreign_or_missing_thread_404(client):
    ...
    signup_and_login(client, email="two@test.com")
    assert (
        client.post("/api/chats", json={"question": "hi", "thread_id": foreign}).status_code == 404
    )
    assert (
        client.post("/api/chats", json={"question": "hi", "thread_id": 999999}).status_code == 404
    )
    # 404는 AI 호출 전(비용 방어) — 페이크 AI도 호출되지 않는다
    assert client.get("/api/users/me/chats").json() == []
...
def test_context_scoped_per_thread(client, fake_ai):
    ...
    signup_and_login(client)
    # 기본 대화에 2개 질문
    client.post("/api/chats", json={"question": "기본 대화의 질문 하나"})
...
def test_context_within_thread_still_last_5(client, fake_ai):
    ...
    client.post("/api/chats", json={"question": "지금까지 뭐라고 했지?", "thread_id": tid})
...
def test_my_chats_thread_filter(client, fake_ai):
    ...
    all_logs = client.get("/api/users/me/chats").json()
