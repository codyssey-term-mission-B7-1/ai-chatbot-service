...
def test_signup_login_me_logout_flow(client):
    ...
    # 로그인 → 세션 쿠키 발급
    r = client.post("/api/session", json={"email": "a@test.com", "password": "Test1234!"})
    assert r.status_code == 201
...
def test_wrong_password_returns_401(client):
    client.post("/api/users", json={"email": "b@test.com", "password": "Test1234!"})
    r = client.post("/api/session", json={"email": "b@test.com", "password": "wrong-pass"})
    assert r.status_code == 401


def test_signup_validation_short_password_422(client):
    r = client.post("/api/users", json={"email": "c@test.com", "password": "1234"})
...
def test_stale_session_after_reseed_returns_401(client, db):
    ...
    assert client.get("/api/users/me").json()["email"] == "test@test.com"
...
def test_email_case_insensitive_signup_login(client):
    ...
    r = client.post("/api/users", json={"email": "Case@Test.com", "password": "Test1234!"})
    assert r.status_code == 201
    ...
    r = client.post("/api/session", json={"email": "CASE@TEST.COM", "password": "Test1234!"})
    assert r.status_code == 201
...
def test_session_cookie_expiry_is_one_day_by_default(client):
    ...
    r = client.post("/api/session", json={"email": "exp@example.com", "password": "Test1234!"})
...
def test_signup_whitespace_only_password_422(client):
    ...
    for blank in ["        ", "\n\n\n\n\n\n\n\n", "\t \t \t \t "]:
        r = client.post("/api/users", json={"email": "blank-pw@test.com", "password": blank})
        assert r.status_code == 422
...
def test_legacy_hash_login_rehashes_to_peppered(client, db):
    ...
    r = client.post("/api/session", json={"email": email, "password": "LegacyPass123!"})
    assert r.status_code == 201
...
def test_login_blank_email_or_password_shows_422_or_401(client):
    ...
    r = client.post("/api/session", json={"email": "   ", "password": "   "})
...
def test_unmarked_peppered_hash_gets_marked_on_login(client, db):
    ...
    r = client.post("/api/session", json={"email": email, "password": "Unmarked123!"})
    assert r.status_code == 201
    db.expire_all()
    refreshed = db.query(User).filter_by(email=email).one()
